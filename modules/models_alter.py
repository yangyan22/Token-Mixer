...
class Model(nn.Module):
    ...
    def forward(self, images, targets=None, tok=None, mode='train', tags=0, epoch_id=0):
        ...
        if mode == 'train':
            ...
            #     replace = random.sample(range(0, 9), 5)
            #     for i in replace:
            #         feats[:, i, :] = att_feats_0[:, i, :]
            #     output_re_t = self.encoder_decoder(feats, targets, mode='forward')
            #     return output_re_t
            RoundGap = 4
            if epoch_id % RoundGap == 0:
                a = int(epoch_id/RoundGap)
                feats = att_feats_0
            ...
            if epoch_id % RoundGap == RoundGap -1:
                a = int((epoch_id+1)/RoundGap)
                feats = sturctured_emb_0
                replace = random.sample(range(0, 9), a)
                for i in replace:
                    feats[:, i, :] = att_feats_0[:, i, :]
